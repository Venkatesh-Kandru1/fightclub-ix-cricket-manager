...
function App() {
  ...
  const visiblePlayers = useMemo(
    () => activeRole === "All" ? players : players.filter((player) => player.role === activeRole),
    [activeRole, players],
  );
...
function PlayerModal({ player, onClose, onChangeStat }: { player: Player; onClose: () => void; onChangeStat: (stat: "runs" | "wickets" | "catches") => void }) {
  const statName = player.role === "Batsman" ? "runs" : player.role === "Bowler" ? "wickets" : "catches";
  const statLabel = statName === "runs" ? "run" : statName === "wickets" ? "wicket" : "catch";
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><article className="modal-card player-modal" role="dialog" aria-modal="true" aria-labelledby="player-modal-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" onClick={onClose} aria-label="Close player details">×</button><div className="modal-player-heading"><span className={`avatar avatar-large role-${player.role.toLowerCase().replace("-", "")}`}>{player.initials}</span><div><span className="kicker">{player.role}</span><h2 id="player-modal-title">{player.name}</h2><p>{player.email}</p></div></div><div className="role-description"><strong>{player.role} duties</strong><p>{player.role === "Batsman" ? "Build the innings, rotate the strike, and anchor the team score." : player.role === "Bowler" ? "Set the field, control the run rate, and take wickets at key moments." : player.role === "All-rounder" ? "Contribute with both bat and ball whenever the team needs balance." : "Lead the gloves, take catches, and organise the field from behind the stumps."}</p></div><div className="modal-stats"><span><b>{player.runs}</b><small>Runs</small></span><span><b>{player.wickets}</b><small>Wickets</small></span><span><b>{player.catches}</b><small>Catches</small></span></div><div className="modal-footer"><span>Joined {formatDate(player.joinedOn)}</span><button className="primary-button compact" type="button" onClick={() => onChangeStat(statName as "runs" | "wickets" | "catches")}>Record {statLabel}</button></div></article></div>;
}
